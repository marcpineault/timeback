...
import { runFFmpegWithRetry, runFFmpegSpawn, FFmpegProcessError, FFmpegProcessConfig } from './ffmpegProcess';
import {
  detectSilenceAdaptive as _detectSilenceAdaptive,
  detectSilence,
...
// Re-export silence detection types and functions for backward compatibility
export { detectSilence, getNonSilentSegments, calculateAdaptiveThreshold };
...
/**
 * Adaptive silence detection — delegates to silenceDetection module.
 * Wraps the module function to inject getVideoDuration dependency.
 */
export async function detectSilenceAdaptive(
  inputPath: string,
  minDuration: number = 0.3
): Promise<AdaptiveResult> {
  return _detectSilenceAdaptive(inputPath, minDuration, getVideoDuration);
}
