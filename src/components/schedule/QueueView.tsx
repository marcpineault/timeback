    ...
    <div>
      ...
      {stats && (
        <div className="flex items-center gap-6 mb-6 text-sm">
          <div>
            <span className="text-[#8a8580]">Scheduled: </span>
            <span className="text-[#0a0a0a] font-medium">{stats.scheduled}</span>
          </div>
          <div>
            <span className="text-[#8a8580]">Published: </span>
            <span className="text-green-400 font-medium">{stats.published}</span>
          </div>
          {stats.failed > 0 && (
            <div>
              <span className="text-[#8a8580]">Failed: </span>
              <span className="text-red-400 font-medium">{stats.failed}</span>
            </div>
          )}
          {stats.nextPostAt && (
            <div>
              <span className="text-[#8a8580]">Next post: </span>
      ...
      {queue.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-[#8a8580] mb-2">No posts in queue</p>
        ...
        <div className="space-y-6">
          {Object.entries(grouped).map(([dateLabel, posts]) => (
            <div key={dateLabel}>
              <h4 className="text-[#8a8580] text-sm font-medium mb-3">
